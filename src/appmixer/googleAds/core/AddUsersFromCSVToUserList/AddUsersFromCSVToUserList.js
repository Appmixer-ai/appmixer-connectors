...
/**
 * How often (ms) to emit a progress update while uploading.
 */
...
function buildOperation(row, schemaConfig, adPersonalization, adUserData) {
    ...
    // uploadMode REPLACE means we remove old entries before adding new ones;
    // from a per-row perspective we always "create" (ADD) here – the job-level
    // upload key controls the replace behaviour.
...
async function createOfflineUserDataJob(context, {
    customerId, userListResourceName, uploadMode
}) {
    const normCustomerId = normId(customerId);
    const headers = lib.buildHeaders(context);
    ...
    if (uploadMode === 'REPLACE') {
        jobBody.job.customerMatchUserListMetadata.uploadKeyType = 'CONTACT_INFO';
    }
...
async function addOperations(context, { customerId, jobResourceName, operations }) {
...
async function runOfflineUserDataJob(context, { customerId, jobResourceName }) {
    const headers = lib.buildHeaders(context);
    ...
    async receive(context) {
        ...
        if (isContinuation) {
            ...
            state = {
                ...
                errors: [],
                internalErrors: [],
                jobs: {},               // { userListResourceName: jobResourceName }
                jobsRun: [],            // resource names of jobs already run (for partial re-run avoidance)
                jobRunning: null        // resource name of the currently open job (not yet run)
            };
        }
        ...
        // Resolve / create the job for this user list (one per upload)
        let jobResourceName = state.jobs[state.userListResourceName] || null;
        if (!jobResourceName) {
            ...
            state.jobs[state.userListResourceName] = jobResourceName;
            state.jobRunning = jobResourceName;
            await context.log({
                step: 'created-job',
                jobResourceName,
                userListResourceName: state.userListResourceName
        ...
        // Read one chunk at a time and upload
        let lastProgressMs = Date.now();

        while (state.lastProcessedRow < state.totalRows) {
            if (isUploadExpired(state.uploadStartMs, context)) {
                const msg = `Upload exceeded maximum allowed time and was aborted after ${state.totalUsersUploaded} users.`;
                state.internalErrors.push(msg);
                await context.log({ step: 'upload-expired', totalUsersUploaded: state.totalUsersUploaded });
                break;
            }
            ...
            state.totalUsersUploaded += operations.length;
            ...
            if (now - lastProgressMs >= PROGRESS_INTERVAL_MS) {
                ...
                await context.sendJson({
                    status: 'in-progress',
                    message: `Uploaded ${state.totalUsersUploaded} users (${pct}%)`,
                    totalRows: state.totalRows,
                    lastProcessedRow: state.lastProcessedRow,
                    totalUsersUploaded: state.totalUsersUploaded,
                    percentage: pct,
        ...
        // ------------------------------------------------------------------
        // 3. Run the job to trigger Google Ads processing
        // ------------------------------------------------------------------
        ...
        // ------------------------------------------------------------------
        // 4. Clean up continuation state and emit final output
        // ------------------------------------------------------------------
        ...
        await context.sendJson({
            totalUsers: state.totalUsersUploaded,
