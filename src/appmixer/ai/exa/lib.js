...
const DEFAULT_PREFIX = 'exa-objects-export';
...
module.exports = {
     ...
     * Exa API base and refuse anything that would send the account's API key
     * somewhere else.
     *
     ...
     * @param {string} url relative path (e.g. '/search') or absolute Exa URL
     * @returns {string} absolute URL on the Exa API host
     */
    ...
    /**
     * Thin wrapper around context.httpRequest that applies the Exa API key header
     * and returns the parsed response body. context.httpRequest throws on non-2xx
     * responses, so callers can rely on the body being present.
     * @param {object} args
     * @param {object} args.context Appmixer component context (needs `auth.apiKey`)
     * @param {string} [args.method] HTTP method (default POST — every Exa data endpoint is POST)
     * @param {string} args.path API path starting with a slash (e.g. '/search')
     * @param {object} [args.data] JSON request body
     * @returns {Promise<object>} the parsed response body
     */
    async makeRequest({ context, method = 'POST', path, data = null }) {
     ...
     * request body entirely rather than sent as an empty array.
    ...
    /**
     * Build the `contents` request object shared by the search and find-similar
     * components. Returns undefined when the user asked for no page content, so
     * the key can be left out of the request body entirely.
     * @param {object} args
     * @param {boolean} [args.includeText] request the cleaned page text
     * @param {boolean} [args.includeHighlights] request LLM-picked relevant snippets
     * @param {string} [args.summaryQuery] request a summary focused on this question
     * @returns {object|undefined}
     */
    buildContentsOptions({ includeText, includeHighlights, summaryQuery }) {
