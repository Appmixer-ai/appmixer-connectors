...
describe('AddTeammatesToTeam Component', function() {

    let component;

    beforeEach(function() {
        component = require('../../../src/appmixer/front/team/AddTeammatesToTeam/AddTeammatesToTeam.js');
    });
    ...
    it('should make POST request to correct URL', async function() {
        const mockResponse = { success: true };
    ...
    it('should send response to out port', async function() {
        ...
        assert(outputData.success === true, 'Should indicate success');
        assert.deepEqual(outputData.teammates, ['tmt_123', 'tmt_456'], 'Should include teammate IDs');
        assert(outputData.team_id === 'tea_123', 'Should include team ID');
        assert(typeof outputData === 'object', 'Should send response data to output');
