...
describe('FindPresentations Component', function() {
    ...
    it('should find presentations without folder filter or search query', async function() {
        let data;
    ...
    it('should handle search query functionality', async function() {
        let data;
        context.sendJson = function(output, port) {
            data = output;
        };
        ...
        try {
            ...
            console.log('FindPresentations with search query result:', JSON.stringify(data, null, 2));
            ...
            // If results found, verify they contain the search term (case insensitive)
            if (data.result.length > 0) {
                const presentation = data.result[0];
                assert(presentation.name, 'Expected presentation to have name property');
                // Note: Google Drive search is flexible, so we don't strictly require the exact match
                console.log(`Found presentation with name: ${presentation.name}`);
            } else {
                console.log('No presentations found matching search query "Test"');
            }
        } catch (error) {
            if (error.response && error.response.status === 401) {
                console.log('Authentication failed - access token may be expired');
    ...
    it('should handle empty search query gracefully', async function() {
        let data;
        context.sendJson = function(output, port) {
            data = output;
        };
        ...
        try {
            ...
            console.log('FindPresentations with empty search query result count:', data ? data.count : 'undefined');

            assert(data && typeof data === 'object', 'Expected sendJsonData.data to be an object');
            assert(Array.isArray(data.result), 'Expected sendJsonData.data.result to be an array');
            assert(typeof data.count === 'number', 'Expected sendJsonData.data.count to be a number');

            // Empty search query should return all presentations (same as no search query)
    ...
    it('should handle search query with special characters', async function() {
        let data;
        context.sendJson = function(output, port) {
            data = output;
        };
        ...
        try {
            ...
            console.log('FindPresentations with special characters search result count:', data ? data.count : 'undefined');

            assert(data && typeof data === 'object', 'Expected sendJsonData.data to be an object');
            assert(Array.isArray(data.result), 'Expected sendJsonData.data.result to be an array');
            assert(typeof data.count === 'number', 'Expected sendJsonData.data.count to be a number');

            console.log('Search query with special characters handled correctly');
        } catch (error) {
            if (error.response && error.response.status === 401) {
                console.log('Authentication failed - access token may be expired');
                console.log('Error details:', error.response.data);
    ...
    it('should handle empty results gracefully', async function() {
        ...
        // Use a search query that likely won't match any presentations
        context.messages.in.content = {
            searchQuery: 'nonexistent-presentation-name-12345',
            outputType: 'array'
        };
