...
describe('Retool FindApps', function() {
    let context;
    ...
    it('should find apps successfully', async function() {
        this.timeout(10000);
    ...
    it('should handle different output types', async function() {
        ...
        // Test first output type (if apps exist)
        await context.sendJson({
            search: '',
            outputType: 'first'
        }, 'in');

        // This might throw a CancelError if no apps exist, which is expected
        try {
            ...
            // If no apps exist, expect CancelError
            assert(error.message.includes('No records available'), 'Should throw appropriate error when no apps exist');
        }
    });
});
