...
describe('FindCampaigns Component', function() {
    ...
    it('should find campaigns with default limit', async function() {
    ...
    it('should find campaigns with custom limit', async function() {
        context.messages.in.content = {
            outputType: 'array',
            limit: 50
        };
    ...
    it('should respect limit parameter cap of 100', async function() {
        ...
        if (port === 'out') {
            if (context.messages.in.content.outputType === 'array') {
                ...
                assert(data.result.length <= 100, 'Should not return more than 100 campaigns due to limit cap');
            } else {
                assert(Array.isArray(data), 'Data should be an array for non-array outputType');
                assert(data.length <= 100, 'Should not return more than 100 campaigns due to limit cap');
            }
        }
    });
});