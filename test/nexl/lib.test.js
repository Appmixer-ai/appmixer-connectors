...
const { sendArrayOutput } = require('../../src/appmixer/nexl/lib');
...
describe('nexl lib', () => {
    ...
    describe('sendArrayOutput', () => {
        ...
        it('should deliver array output to the out port', async () => {
            ...
            assert.deepStrictEqual(jsonCalls[0].payload, { contacts: sampleRecords, count: sampleRecords.length }, 'should format array payload correctly');
        });
        ...
        it('should write files when outputType is file', async () => {
            ...
            assert.deepStrictEqual(jsonCalls[jsonCalls.length - 1].payload, { fileId: 'file-123', count: sampleRecords.length }, 'should report saved file');
        });
